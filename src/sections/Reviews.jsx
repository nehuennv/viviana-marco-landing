...
const Reviews = () => {
  ...
  // Lógica de scroll (Mantenemos la que tenías si te funciona bien, 
  // aunque IntersectionObserver es más ligero, esto está ok para pocas reviews)
  const handleScroll = () => {
    if (!ticking.current) {
      window.requestAnimationFrame(() => {
        if (scrollRef.current) {
          ...
          // Ajustamos el loop para ignorar el div espaciador del final si existe
          ...
          {displayReviews.map((review, idx) => {
            ...
            return (
              <motion.div
                ...
                animate={{
                  opacity: 1,
                  // Escala solo en móvil para indicar foco, en desktop fija
                ...
                className="
                  ...
                  /* Quitamos padding vertical extra que causaba problemas */
                "
              >
                {/* CARD OPTIMIZADA (Igual que ServiceCard):
                   - Fondo: Gradiente en móvil, Blanco en desktop.
                   - Borde: Violeta sutil en móvil, Gris en desktop.
                   - Sombra: Ninguna en móvil (evita cortes), Suave en desktop.
                   - Hover: Desactivado en móvil, Activo en desktop.
                */}
                <div className="
                    ...
                    md:hover:shadow-2xl md:hover:shadow-violet-500/20 
                    md:hover:-translate-y-1
                  ">

                  {/* Decoración Fondo (Shimmer) */}
                  ...
                  {/* Icono Comillas */}
                  <div className="absolute top-6 right-6 transition-transform duration-500 ease-out md:group-hover:rotate-12 hidden md:block">
                    <Quote className="text-slate-200/50 transition-colors duration-500 md:group-hover:text-violet-200" size={60} fill="currentColor" />
                  </div>
                  ...
                  {/* FOOTER TARJETA */}
                  <div className="mt-6 md:mt-8 pt-5 border-t border-slate-100/50 flex items-center justify-between relative z-10 text-slate-400/80 transition-colors duration-300 md:group-hover:border-violet-100">
                    <div className="flex items-center gap-2 filter grayscale opacity-70 transition-all duration-300 md:group-hover:grayscale-0 md:group-hover:opacity-100">
                      ...
                      <span className="text-xs font-bold md:group-hover:text-slate-600 transition-colors">Google</span>
                    </div>
                    <span className="text-xs font-medium md:group-hover:text-slate-500 transition-colors">{review.date}</span>
                  </div>
                </div>
              </motion.div>
            );
